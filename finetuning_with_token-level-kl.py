...
TRAIN_DATASET_ID = "LLM-LAT/benign-dataset"
#TRAIN_DATASET_ID = "LLM-LAT/harmful-dataset"
EVAL_DATASET_ID = "allenai/real-toxicity-prompts"
...
NUM_EPOCHS = 3
LEARNING_RATE = 2e-5
OUTPUT_DIR_KL = "./finetuned_kl_regularized_danube"
...
if torch_dtype == torch.float16:
    print("  Note: Gradient checkpointing disabled for float16 stability")
else:
    try:
...
try:
    train_dataset = load_dataset(TRAIN_DATASET_ID, split="train[:50]")
except:
    train_dataset = load_dataset("tatsu-lab/alpaca", split="train[:50]")
...
try:
    trainer_kl.train()
    trainer_kl.save_model(OUTPUT_DIR_KL)
    print(f"\n✓ KL-regularized model saved to {OUTPUT_DIR_KL}")
except Exception as e:
